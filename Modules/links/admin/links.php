...
global $language, $links_DB, sql_prefix('');
...
if (autorisation(-127)) {
    $result = sql_query("SELECT radminsuper FROM " . sql_prefix('') . "authors WHERE aid='$aid'");
...
function links()
{
    global $ModPath, $ModStart, $links_DB, $admin, $language, sql_prefix(''), $hlpfile;
    include("header.php");
...
{
    global $ModPath, $ModStart, $links_DB, $hlpfile, sql_prefix('');
    include("header.php");
    echo helpwindow();
...
{
    global $ModPath, $ModStart, $links_DB, sql_prefix('');
    ...
    while (list($requestid, $lid, $cid, $sid, $title, $url, $description, $modifysubmitter, $topicid_card) = sql_fetch_row($resultX)) {
        ...
        $result7 = sql_query("SELECT email FROM " . sql_prefix('') . "users WHERE uname='$modifysubmitter'");
        $result8 = sql_query("SELECT email FROM " . sql_prefix('') . "users WHERE uname='$owner'");
        $result9 = sql_query("SELECT topictext FROM " . sql_prefix('') . "topics WHERE topicid='$oritopicid_card'");
        $result9b = sql_query("SELECT topictext FROM " . sql_prefix('') . "topics WHERE topicid='$topicid_card'");
    ...
    <table id="tad_linkbrok" data-toggle="table" data-striped="true" data-search="true" data-show-toggle="true" data-mobile-responsive="true" data-icons="icons" data-icons-prefix="fa">
        ...
        while (list($requestid, $lid, $modifysubmitter) = sql_fetch_row($resultBrok)) {
            ...
            if ($modifysubmitter != '$anonymous') {
                $result3 = sql_query("SELECT email FROM " . sql_prefix('') . "users WHERE uname='$modifysubmitter'");
                list($email) = sql_fetch_row($result3);
            }
            list($title, $url, $owner) = sql_fetch_row($result2);
            $result4 = sql_query("SELECT email FROM " . sql_prefix('') . "users WHERE uname='$owner'");
