...
function SearchForm()
{
    global $ModPath, $ModStart, sql_prefix(''), $links_topic;
    echo '
...
{
    global $ModPath, $ModStart, $links_DB, sql_prefix(''), $user, $admin;
    if ($admin) {
        $Xadmin = base64_decode($admin);
        $Xadmin = explode(':', $Xadmin);
        $result = sql_query("SELECT radminsuper FROM " . sql_prefix('') . "authors WHERE aid='$Xadmin[0]'");
