...
if (autorisation(-127)) {
    ...
    if ($geo_ip == 1) {
        $arpostip = array();
        $result_postip = sql_query('SELECT COUNT(*) AS nbpost , poster_ip FROM ' . sql_prefix('') . 'posts GROUP BY poster_ip');
        ...
        $spamip = array();
        $spamipfile = file("storage/slogs/spam.log");
        ...
        $result_ip = sql_query('SELECT * FROM ' . sql_prefix('') . 'ip_loc ORDER BY ip_visite DESC');
...
if (isset($cookie)) {
    $result = sql_query('SELECT uid FROM ' . sql_prefix('') . 'users WHERE uname LIKE "' . $username . '"');
    $row = sql_fetch_array($result);
    $uid = $row['uid'];
    // voir si user existe dans users_extend
    $resul = sql_query('SELECT uid FROM ' . sql_prefix('') . 'users_extend WHERE uid = "' . $uid . '"');
    $found = sql_num_rows($resul);
    //mise à jour users_extend si besoin
    if ($found == 0)
        $res = sql_query("INSERT INTO " . sql_prefix('') . "users_extend VALUES ('$uid','','','','','','','','','','','','','')");
}
//==> georeferencement utilisateur
if (array_key_exists('mod', $_GET)) {
    if ($f_new_lat != '' and $f_new_long != '' and $f_geomod = "neo")
        sql_query('UPDATE ' . sql_prefix('') . 'users_extend SET ' . $ch_lat . ' = "' . $f_new_lat . '", ' . $ch_lon . ' = "' . $f_new_long . '" WHERE uid = "' . $uid . '"');
    if ($f_new_lat != '' and $f_new_long != '' and $f_geomod = "mod")
        sql_query('UPDATE ' . sql_prefix('') . 'users_extend SET ' . $ch_lat . ' = "' . $f_new_lat . '", ' . $ch_lon . ' = "' . $f_new_long . '" WHERE uid = "' . $f_uid . '"');
}

$result = sql_query('SELECT * FROM ' . sql_prefix('') . 'users u LEFT JOIN ' . sql_prefix('') . 'users_extend ue ON u.uid = ue.uid WHERE uname LIKE "' . $username . '"');
while ($row = sql_fetch_array($result)) {
    ...
    else $the_av_ch = 'public/assets/images/forum/avatar/' . $user_avatar;
}
//<== georeferencement utilisateur
...
$result_con = sql_query('SELECT * FROM ' . sql_prefix('') . 'session s');
$total_connect = sql_num_rows($result_con);

// récupération des infos des connectés (membres et ano)
$result = sql_query('SELECT * FROM ' . sql_prefix('') . 'session s
    LEFT JOIN ' . sql_prefix('') . 'users u ON s.username = u.uname
    LEFT JOIN ' . sql_prefix('') . 'users_extend ue ON u.uid = ue.uid');
$krs = 0;
$mb_con_g = '#######';
...
while ($row = sql_fetch_array($result)) {
    ...
    $av_ch = preg_match('#\/#', $users_user_avatar) === 1 ?
        $users_user_avatar :
        'public/assets/images/forum/avatar/' . $users_user_avatar;
    ...
    */
    ...
    $tres = sql_query("SELECT * FROM " . sql_prefix('') . "ip_loc i WHERE ip_ip LIKE \"$session_host_addr\"");
...
//==>les membres
$mbgr = 0;
$membre = sql_query('SELECT * FROM ' . sql_prefix('') . 'users u LEFT JOIN ' . sql_prefix('') . 'users_extend ue ON u.uid = ue.uid ORDER BY u.uname');
...
while ($row = sql_fetch_array($membre)) {
    ...
    else $av_ch = 'public/assets/images/forum/avatar/' . $us_user_avatar;
...
if (!defined('OL')) {
    define('OL', 'ol');
    $ecr_scr = '
    <script type="text/javascript" src="' . $nuke_url . '/public/assets/shared/ol/ol.js"></script>';
}
$ecr_scr .= '
    ...
    if (!$("link[href=\'/public/assets/shared/ol/ol.css\']").length)
        $("head link[rel=\'stylesheet\']").last().after("<link rel=\'stylesheet\' href=\'' . $nuke_url . '/public/assets/shared/ol/ol.css\' type=\'text/css\' media=\'screen\'>");
    ...
    if (!$("link[href=\'public/assets/shared/bootstrap/dist/css/bootstrap-icons.css\']").length)
        $("head link[rel=\'stylesheet\']").last().after("<link rel=\'stylesheet\' href=\'' . $nuke_url . '/public/assets/shared/bootstrap/dist/css/bootstrap-icons.css\' type=\'text/css\' media=\'screen\'>");
