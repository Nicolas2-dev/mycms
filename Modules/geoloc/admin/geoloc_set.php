...
{
    global sql_prefix('');
    $sql = "DELETE FROM " . sql_prefix('') . "ip_loc WHERE ip_id >=1";
    if ($result = sql_query($sql))
        sql_query("ALTER TABLE " . sql_prefix('') . "ip_loc AUTO_INCREMENT = 0;");
}

function Configuregeoloc($subop, $ModPath, $ModStart, $ch_lat, $ch_lon, $cartyp, $geo_ip, $api_key_ipdata, $key_lookup)
{
    global $hlpfile, $language, $f_meta_nom, $f_titre, $adminimg, $dbname, sql_prefix(''), $subop;
    include('modules/' . $ModPath . '/geoloc.conf');
    $hlpfile = 'modules/' . $ModPath . '/doc/aide_admgeo_' . $language . '.html';

    $result = sql_query("SELECT CONCAT(ROUND(((DATA_LENGTH + INDEX_LENGTH - DATA_FREE) / 1024 / 1024), 2), ' Mo') AS TailleMo FROM information_schema.TABLES WHERE TABLE_SCHEMA = '$dbname' AND TABLE_NAME = " . sql_prefix('') . "'ip_loc'");
    ...
    $scri = '';
    $scri .= '
    <script type="text/javascript" src="public/assets//js/jscolor.min.js"></script>
    <script type="module">
    //<![CDATA[
    $(function() {
        if (typeof ol=="undefined")
            $("head").append($("<script />").attr({"type":"text/javascript","src":"public/assets/shared/ol/ol.js"}));
        $("head").append($("<script />").attr({"type":"text/javascript","src":"modules/geoloc/include/fontawesome.js"}));
        $("head link[rel=\'stylesheet\']").last().after("<link rel=\'stylesheet\' href=\'/public/assets/shared/ol/ol.css\' type=\'text/css\' media=\'screen\'>");
