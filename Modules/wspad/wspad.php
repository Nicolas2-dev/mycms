...
global $title, $language, sql_prefix(''), $user, $admin, $nuke_url;
// For More security
...
function Liste_Page()
{
    global sql_prefix(''), $ModPath, $ModStart, $ThisFile, $gmt, $auteur, $groupe, $couleur;
    echo '
    ...
    $nb_pages = sql_num_rows(sql_query("SELECT COUNT(page) FROM " . sql_prefix('') . "wspad WHERE member='$groupe' GROUP BY page"));
    if ($groupe > 0) {
        $gp = sql_fetch_assoc(sql_query("SELECT groupe_name FROM " . sql_prefix('') . "groupes WHERE groupe_id='$groupe'"));
    ...
    if ($nb_pages > 0) {
        $ibid = 0;
        $pgibid = 0;
        $result = sql_query("SELECT DISTINCT page FROM " . sql_prefix('') . "wspad WHERE member='$groupe' ORDER BY page ASC");
        while (list($page) = sql_fetch_row($result)) {
            ...
            if (file_exists($filename)) {
                if ((time() - $refresh) > filemtime($filename)) {
                    sql_query("UPDATE " . sql_prefix('') . "wspad SET verrou='' WHERE page='$page' AND member='$groupe'");
                    @unlink($filename);
                    $verrou = '';
                }
            }
            ...
            $result2 = sql_query("SELECT modtime, editedby, ranq, verrou FROM " . sql_prefix('') . "wspad WHERE page='$page' AND member='$groupe' ORDER BY ranq ASC");
...
{
    global sql_prefix(''), $ModPath, $ModStart, $gmt, $auteur, $groupe, $mess;
    $tmp = "
    <script type='text/javascript'>
        //<![CDATA[
    ...
    if (file_exists($filename)) {
        if (filemtime($filename) > (time() - $refresh)) {
            ...
            fwrite($fp, $auteur);
            fclose($fp);
            sql_query("UPDATE " . sql_prefix('') . "wspad SET verrou='' WHERE verrou='$auteur'");
            sql_query("UPDATE " . sql_prefix('') . "wspad SET verrou='$auteur' WHERE page='$page' AND member='$groupe'");
            $edition = true;
            echo $tmp;
        }
    } else {
    ...
    $row = sql_fetch_assoc(sql_query("SELECT content, modtime, editedby, ranq FROM " . sql_prefix('') . "wspad WHERE page='$page' AND member='$groupe' AND ranq='$ranq'"));
    if (!$edition)
...
switch ($op) {
    case "sauve":
        ...
        $row = sql_fetch_assoc(sql_query("SELECT MAX(ranq) AS ranq FROM " . sql_prefix('') . "wspad WHERE page='$page' AND member='$groupe'"));
        $result = sql_query("INSERT INTO " . sql_prefix('') . "wspad VALUES ('0', '$page', '$content', '" . time() . "', '$auteur', '" . ($row['ranq'] + 1) . "', '$groupe','')");
        sql_query("UPDATE " . sql_prefix('') . "wspad SET verrou='' WHERE verrou='$auteur'");
    ...
    case "supp":
        $auteur = removeHack(stripslashes(FixQuotes($auteur)));
        $result = sql_query("DELETE FROM " . sql_prefix('') . "wspad WHERE page='$page' AND member='$groupe' AND ranq='$ranq'");
        sql_query("UPDATE " . sql_prefix('') . "wspad SET verrou='' WHERE verrou='$auteur'");
        break;
    case "suppdoc":
        settype($member, 'integer');
        $result = sql_query("DELETE FROM " . sql_prefix('') . "wspad WHERE page='$page' AND member='$member'");
        @unlink("modules/$ModPath/locks/$page-vgp-$groupe.txt");
        break;
    case "renomer":
        // Filtre les caractères interdits dans les noms de pages
        ...
        settype($member, 'integer');
        $result = sql_query("UPDATE " . sql_prefix('') . "wspad SET page='$newpage', verrou='' WHERE page='$page' AND member='$member'");
        @unlink("modules/$ModPath/locks/$page-vgp-$groupe.txt");
        break;
    case "conv_new":
        $row = sql_fetch_assoc(sql_query("SELECT content FROM " . sql_prefix('') . "wspad WHERE page='$page' AND member='$groupe' AND ranq='$ranq'"));
        $date_debval = date("Y-d-m H:i:s", time());
        $deb_year = substr($date_debval, 0, 4);
        $date_finval = ($deb_year + 99) . "-01-01 00:00:00";
        $result = sql_query("INSERT INTO " . sql_prefix('') . "queue VALUES (NULL, $cookie[0], '$auteur', '$page', '" . FixQuotes($row['content']) . "', '', now(), '','$date_debval','$date_finval','0')");
        break;
}
