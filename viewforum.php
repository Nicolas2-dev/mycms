        ...
        while (list($topic_id) = sql_fetch_row($resultT)) {
            ...
            if ($r) {
                if (!list($rid) = sql_fetch_row($r)) {
                    $r = sql_query("INSERT INTO " . sql_prefix('forum_read') . " (forum_id, topicid, uid, last_read, status) 
                                    VALUES ('$forum', '$topic_id', '$userR[0]', $time_actu, '1')");
                } else {
                    $r = sql_query("UPDATE " . sql_prefix('forum_read') . " 
...
if (($myrow['forum_type'] == 1) and (($myrow['forum_name'] != $forum_name) or ($Forum_passwd != $myrow['forum_pass']))) {
    ...
    for ($i = 0; $i < count($moderator_data); $i++) {
        $modera = User::get_userdata($moderator_data[$i]);

        if ($modera['user_avatar'] != '') {
            if (stristr($modera['user_avatar'], "users_private")) {
                $imgtmp = $modera['user_avatar'];
            } else {
                $imgtmp = ($ibid = Theme::image("forum/avatar/" . $modera['user_avatar'])) 
                    ? $ibid 
                    : "public/assets/images/forum/avatar/" . $modera['user_avatar'];
            }
        }
    ...
    $sql = "SELECT * FROM " . sql_prefix('forumtopics') . " 
            WHERE forum_id='$forum' $closol 
            ORDER BY topic_first, topic_time 
            DESC LIMIT $start, $topics_per_page";
    ...
    if ($ibid = Theme::image("forum/icons/posticon.gif")) {
        ...
        $imgtmpP = "public/assets/images/forum/icons/posticon.gif";
    }
    ...
    if ($myrow = sql_fetch_assoc($result)) {
        ...
        do {
            ...
            if ($replys >= 0) {
                ...
                if ($smilies) {
                    $rowQ1 = Cache::Q_Select("SELECT image 
                ...
                if ($image_subject != '') {
                    if ($ibid = Theme::image("forum/subject/$image_subject")) {
                        $imgtmp = $ibid;
                    } else {
                        $imgtmp = "public/assets/images/forum/subject/$image_subject";
                    }
