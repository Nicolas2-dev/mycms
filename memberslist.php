...
class MemberlistController extends Controller
{
     ...
     * @param   string  $list        [$list description]
     * @param   mixed   $gr_from_ws  [$gr_from_ws description]
     *
     * @return  void
     */
    public function index(?string $letter, ?string $sortby, ?string $list, mixed $gr_from_ws)
    {
        global $admin, $user;
        ...
        $result = sql_query("SELECT u.uname, u.user_avatar 
                             ...
                             LEFT JOIN " . sql_prefix('users_status') . " AS us ON u.uid = us.uid where us.open='1' 
                             ORDER BY u.uid DESC LIMIT 0,1");
        
        list($lastuser, $lastava) = sql_fetch_row($result);
        
        echo '<h2><img src="' . asset_url('images/admin/users.png') . '" alt="' . translate("Liste des membres") . '" />' . translate("Liste des membres");
        ...
        if (!isset($gr_from_ws)) {
            ...
            if ($ibid_avatar = $this->avatar($lastava)) {
                echo '
                    <div class="col-md-1">
            ...
            echo '
                    <div class="col">
                        ' . translate("Bienvenue au dernier membre affilié : ") . ' <br />
                        <h4><a href="' . site_url('user.php?op=userinfo&amp;uname=' . $lastuser) . '">' . $lastuser . '</a></h4>
                    </div>
                </div>
                <hr />';
        }
        ...
        $this->alpha($sortby, $list, $gr_from_ws);
        
        echo '</p>';
        
        $sortby = $this->SortLinks($letter, $sortby, $list, $gr_from_ws);
        ...
        // Ajout pour supprimer l'anonyme de la liste des membres, anonyme n'est pas un membre !
        ...
        if ($letter != 'front') {
            ...
            if ($num_rows_per_order > 0) {
                ...
                while ($temp_user = sql_fetch_assoc($result)) {
                    ...
                    if (($open_user == 1 and !Config::get('npds.member_list')) || ($user) || ($admin)) {
                        ...
                        if ($ibid_avatar = $this->avatar($temp_user['user_avatar'])) {
                            ...
                            <td><a href="'. site_url('user.php?op=userinfo&amp;uname=' . $temp_user['uname']) . '" title="' . translate("Inscription") . ' : ' . Date::formatTimes($temp_user['user_regdate'], IntlDateFormatter::SHORT, IntlDateFormatter::MEDIUM);
                        }
                        ...
                        if ($sortby != 'user_from ASC') {
                            if ($admin) {
                                ...
                                if ($temp_user['user_viewemail']) {
                                    ...
                                    echo '<td><small>' . substr($temp_user['femail'], 0, strpos($temp_user['femail'], "@") ?: 0) . '</small></td>';
                                }
                            }
                        } else {
                        ...
                        if ($admin) {
                            ...
                            if (!$temp_user['is_visible']) {
                                echo '<img src="' . asset_url('images/admin/ws/user_invisible.gif') . '" alt="' . translate("Membre invisible") . '" title="' . translate("Membre invisible") . '" />';
                            } else {
                                echo '<img src="' . asset_url('images/admin/ws/blank.gif') . '" alt="" />';
                            }
            ...
            if ($user) {
                ...
                if ($list) {
                    ...
                    <span class="float-end">
                        <a href="'. site_url('replypmsg.php?send=' . substr($list, 0, strlen($list) - 3)) . '" >
                            <i class="far fa-envelope fa-lg" title="' . translate("Ecrire à la liste") . '" data-bs-toggle="tooltip" ></i>
                        </a>
                        <a class="ms-3" href="'. site_url('memberslist.php?letter=' . $letter . '&amp;sortby=' . $sortby . '&amp;page=' . $page . '&amp;gr_from_ws=' . $gr_from_ws) . '" >
                            <i class="fas fa-trash fa-lg text-danger" title="' . translate("Raz de la liste") . '" data-bs-toggle="tooltip" ></i>
                        </a>
                    </span>';
                }
            ...
            if ($num_rows_per_order > $pagesize) {
                ...
                <div class="mt-3 lead align-middle">
                    <span class="badge bg-secondary lead">
                    ' . $num_rows_per_order . '</span> ' . translate("Utilisateurs trouvés pour") . ' <strong>' . $letter . '</strong> (' . $total_pages . ' ' . translate("pages") . ', ' . $num_users . ' ' . translate("Utilisateurs montrés") . ').
                </div>
                ...
                echo '<div class="mt-3 lead align-middle">
                    <span class="badge bg-secondary lead">' . $num_rows_per_order . '</span> ' . translate("Utilisateurs trouvés") . '
                </div>';
            }
        }
...
trait MemberlistPrivate
{
     ...
     *
     * @param   string  $sortby      [$sortby description]
     * @param   mixed   $list        [$list description]
     * @param   mixed   $gr_from_ws  [$gr_from_ws description]
     *
     * @return  [type]               [return description]
     */   
    protected function alpha(string $sortby, mixed $list, mixed $gr_from_ws)
    {
     ...
     * @return  array
     */
    protected function unique(array $ibid)
    {
     ...
     * @return  string
     */
    protected function SortLinks(string $letter, string $sortby, mixed $list, mixed $gr_from_ws)
    {
        global $admin;
     ...
     * @return  string
     */
    protected function avatar(string $user_avatar)
    {
        if (!$user_avatar) {
            $imgtmp = asset_url('images/forum/avatar/blank.gif');

        } else if (stristr($user_avatar, 'users_private')) {
            $imgtmp = $user_avatar;

        } else {
            if ($ibid = Theme::image('forum/avatar/' .$user_avatar)) {
                $imgtmp     = $ibid;
                $imgpath    = $ibid;
            } else {
...
switch (Request::input('op')) 
{
    ...
    default:
        controllerStart(
            MemberlistController::class, 'index',
            [
                // ?string nullable
