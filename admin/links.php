...
function LinksModLink($lid)
{
    ...
    $resulted2 = sql_query("SELECT adminid, editorialtimestamp, editorialtext, editorialtitle 
                            FROM " . sql_prefix('links_editorials') . " 
                            WHERE linkid='$lid'");
...
{
    sql_query("DELETE FROM " . sql_prefix('links_modrequest') . " 
               WHERE lid='$lid'");

    sql_query("DELETE FROM " . sql_prefix('links_links') . " 
               WHERE lid='$lid'");
...
{
    if ($sub == 0) {
        sql_query("UPDATE " . sql_prefix('links_categories') . " 
...
function LinksDelCat($cid, $sid, $sub, $ok = 0)
{
    if ($ok == 1) {
        if ($sub > 0) {
            sql_query("DELETE FROM " . sql_prefix('links_subcategories') . " 
                       WHERE sid='$sid'");
...
{
    sql_query("DELETE FROM " . sql_prefix('links_newlink') . " 
               WHERE lid='$lid'");
...
{
    sql_query("DELETE FROM " . sql_prefix('links_editorials') . " 
               WHERE linkid='$linkid'");
