...
{
    sql_query("UPDATE " . sql_prefix('related') . " 
...
function topicmake($topicname, $topicimage, $topictext, $topicadmin)
{
    ...
    $istopicname = sql_num_rows(sql_query("SELECT * FROM " . sql_prefix('topics') . " 
    ...
    for ($i = 0; $i < count($topicadminX); $i++) {
        ...
        $nres = sql_num_rows(sql_query("SELECT * FROM " . sql_prefix('droits') . " 
                                        WHERE d_aut_aid='$topicadminX[$i]' 
                                        and d_droits=11112"));

        if ($nres == 0) {
            sql_query("INSERT INTO " . sql_prefix('droits') . " 
...
function topicchange($topicid, $topicname, $topicimage, $topictext, $topicadmin, $name, $url)
{
    ...
    $res = sql_query("SELECT * FROM " . sql_prefix('droits') . " 
                      WHERE d_droits=11112 AND d_fon_fid=2");
    ...
    foreach ($topad as $value) { //pour chaque droit adminsujet on regarde le nom de l'adminsujet
        if (!in_array($value, $topicadminX)) { //si le nom de l'adminsujet n'est pas dans les nouveaux adminsujet
            ...
            $resu =  mysqli_get_client_info() <= '8.0' 
                ? sql_query("SELECT * FROM " . sql_prefix('topics') . " 
