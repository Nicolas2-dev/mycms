...
include("Library/Filesystem/File.php");
...
function droits($member)
{
    ...
    echo '<input type="radio" id="adm" name="privs" class="form-check-input" value="-127" ' . $checked . ' />
    ...
    echo '<input type="radio" id="ano" name="privs" class="form-check-input" value="-1" ' . $checked . ' />
    ...
    if ($member > 0) {
        ...
        echo '<input type="radio" id="mem" name="privs" class="form-check-input" value="1" />
                <label class="form-check-label" for="mem">' . adm_translate("Membres") . '</label>
            </div>
            <div class="form-check form-check-inline">
...
function DownloadAdmin()
{
    ...
    $resultX = sql_query("SELECT DISTINCT dcategory 
                          FROM " . sql_prefix('downloads') . " ORDER BY dcategory");
...
function DownloadAdd($dcounter, $durl, $dfilename, $dfilesize, $dweb, $duser, $dver, $dcategory, $sdcategory, $description, $privs, $Mprivs)
{
    ...
    if (($durl) and ($dfilename)) {
        sql_query("INSERT INTO " . sql_prefix('downloads') . " 
...
function DownloadDel($did, $ok = 0)
{
    ...
    if ($ok == 1) {
        sql_query("DELETE FROM " . sql_prefix('downloads') . " 
                   WHERE did='$did'");
