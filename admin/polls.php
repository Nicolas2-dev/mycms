...
function poll_createPosted()
{
    ...
    sql_query("INSERT INTO " . sql_prefix('poll_desc') . " 
    ...
    for ($i = 1; $i <= sizeof($optionText); $i++) {
        ...
        sql_query("INSERT INTO " . sql_prefix('poll_data') . " (pollID, optionText, optionCount, voteID, pollType) 
...
function poll_removePosted()
{
    ...
    if ($setCookies == '1') {
        $sql = "DELETE FROM " . sql_prefix('appli_log') . " 
                WHERE al_id='$al_id' 
                AND al_subid='$id'";
    ...
    sql_query("DELETE FROM " . sql_prefix('poll_desc') . " 
               WHERE pollID='$id'");

    sql_query("DELETE FROM " . sql_prefix('poll_data') . " 
               WHERE pollID='$id'");
...
function poll_SendEditPoll()
{
    ...
    sql_query("UPDATE " . sql_prefix('poll_desc') . " 
    ...
    for ($i = 1; $i <= sizeof($optionText); $i++) {
        ...
        sql_query("UPDATE " . sql_prefix('poll_data') . " 
