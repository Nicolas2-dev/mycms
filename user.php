...
function showimage()
{
    ...
    if ($ibid = theme_image("forum/avatar/blank.gif")) {
        ...
        $imgtmp = "public/assets/images/forum/avatar/";
    }
...
function Only_NewUser()
{
    ...
    if (!$user) {
        ...
        include("Sform/extend-user/extend-user.php");
...
function confirmNewUser($uname, $name, $email, $user_avatar, $user_occ, $user_from, $user_intrest, $user_sig, $user_viewemail, $pass, $vpass, $user_lnl, $C1, $C2, $C3, $C4, $C5, $C6, $C7, $C8, $M1, $M2, $T1, $T2, $B1)
{
    ...
    if (!$stop) {
        ...
        include("Sform/extend-user/aff_extend-user.php");
...
function userinfo($uname)
{
    ...
    if (array_key_exists($ch_lat, $posterdata_extend) and array_key_exists($ch_lon, $posterdata_extend))
        if ($posterdata_extend[$ch_lat] != '' and $posterdata_extend[$ch_lon] != '') {
            ...
            if (!defined('OL')) {
                define('OL', 'ol');
                $content .= '<script type="text/javascript" src="' . $nuke_url . '/public/assets/shared/ol/ol.js"></script>';
            }
...
function edituser()
{
    ...
    include("Sform/extend-user/mod_extend-user.php");
            ...
            if (!$stop) {
                ...
                if ($pass != '') {
                    ...
                    sql_query("UPDATE " . sql_prefix('users') . " 
                               SET name='$name', 
                                   email='$email', 
                                   femail='" . removeHack($femail) . "', 
                                   url='" . removeHack($url) . "', 
                                   pass='$pass', 
                                   hashkey='1', 
                                   bio='" . removeHack($bio) . "', 
                                   user_avatar='$user_avatar', 
                                   user_occ='" . removeHack($user_occ) . "', 
                                   user_from='" . removeHack($user_from) . "', 
                                   user_intrest='" . removeHack($user_intrest) . "', 
                                   user_sig='" . removeHack($user_sig) . "', 
                                   user_viewemail='$a', 
                                   send_email='$u', 
                                   is_visible='$v', 
                                   user_lnl='$w' 
                    ...
                    $result = sql_query("SELECT uid, uname, pass, storynum, umode, uorder, thold, noscore, ublockon, theme, commentmax
                                         FROM " . sql_prefix('users') . " 
                                         WHERE uname='$uname' 
                                         AND pass='$pass'");
