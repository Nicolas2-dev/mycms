...
$rowQ1 = Cache::Q_Select("SELECT forum_id 
...
for ($i = 0; $i < $total_contributeurs; $i++) {
    $contri = User::get_userdata_from_id($contributeurs[$i]);
    ...
    if ($contributeurs[$i] !== '0') {
        if ($contri['user_avatar'] != '') {
            ...
            if (stristr($contri['user_avatar'], "users_private")) {
                ...
                if ($ibid = Theme::image("forum/avatar/" . $contri['user_avatar'])) {
                    $imgtmp = $ibid;
                } else {
                    $imgtmp = "public/assets/images/forum/avatar/" . $contri['user_avatar'];
                }
            }
        }
        ...
        echo '<img class="img-thumbnail img-fluid n-ava-small mb-1" src="public/assets/images/forum/avatar/blank.gif" alt="' . $anonymous . '" title="' . $anonymous . '" data-bs-toggle="tooltip" />';
    }
}
...
for ($i = 0; $i < $ibidcountmod; $i++) {
    $modera = User::get_userdata($moderator[$i]);

    if ($modera['user_avatar'] != '') {
        if (stristr($modera['user_avatar'], "users_private")) {
            $imgtmp = $modera['user_avatar'];
        } else {
            if ($ibid = Theme::image("forum/avatar/" . $modera['user_avatar'])) {
                $imgtmp = $ibid;
            } else {
                $imgtmp = "public/assets/images/forum/avatar/" . $modera['user_avatar'];
            }
        }
    }
...
if ($ibid = Theme::image("forum/icons/posticon.gif")) {
    $imgtmpPI = $ibid;
} else {
    $imgtmpPI = "public/assets/images/forum/icons/posticon.gif";
}

if ($ibid = Theme::image("forum/icons/new.gif")) {
    $imgtmpNE = $ibid;
} else {
    $imgtmpNE = "public/assets/images/forum/icons/new.gif";
}

do {
    $posterdata = User::get_userdata_from_id($myrow['poster_id']);
    if ($myrow['poster_id'] !== '0') {
        ...
        if (!$short_user) {
            $posterdata_extend = User::get_userdata_extend_from_id($myrow['poster_id']);
        ...
        if ($myrow['poster_id'] !== '0') {
            if ($posterdata['user_avatar'] != '') {
                ...
                if (stristr($posterdata['user_avatar'], 'users_private')) {
                    ...
                    if ($ibid = Theme::image('forum/avatar/' . $posterdata['user_avatar'])) {
                        $imgtmp = $ibid;
                    } else {
                        $imgtmp = 'public/assets/images/forum/avatar/' . $posterdata['user_avatar'];
                    }
                }
            }

            echo '
            <a style="position:absolute; top:0.5rem;" tabindex="0" data-bs-toggle="popover" data-bs-trigger="focus" data-bs-html="true" data-bs-title="' . $posterdata['uname'] . '" data-bs-content=\'<div class="my-2 border rounded p-2">' . Forum::member_qualif($posterdata['uname'], $posts, $posterdata['rang']) . '</div><div class="list-group mb-3 text-center">' . $useroutils . '</div><div class="mx-auto text-center" style="max-width:170px;">' . $my_rs . '</div> \'><img class=" btn-outline-primary img-thumbnail img-fluid n-ava" src="' . $imgtmp . '" alt="' . $posterdata['uname'] . '" /></a>
            <span style="position:absolute; left:6em;" class="text-body-secondary"><strong>' . $posterdata['uname'] . '</strong></span>';
        } else {
            echo '<a style="position:absolute; top:0.5rem;" title="' . $anonymous . '" data-bs-toggle="tooltip"><img class=" btn-outline-primary img-thumbnail img-fluid n-ava" src="public/assets/images/forum/avatar/blank.gif" alt="' . $anonymous . '" /></a>
    ...
    if ($myrow['image'] != '') {
        if ($ibid = Theme::image("forum/subject/" . $myrow['image'])) {
            $imgtmp = $ibid;
        } else {
            $imgtmp = "public/assets/images/forum/subject/" . $myrow['image'];
        }
