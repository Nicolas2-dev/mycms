...
class Hack 
{
     ...
     * Instance Hack.
     *
     * @var \Npds\Hack $instance
     */
    protected static Hack $instance;

    /**
     * Instance Editeur.
     *
     * @var Npds\Editeur\Editeur $instance
     */
    ...
    {
        $this->editeur = Editeur::getInstance();
    }

    /**
     * Get instance class Hack.
     *
     * @return \Npds\Hack $instance
     */
     ...
     * @return  string
     */
    public function remove(string $Xstring)
    {
        if ($Xstring != '') {

            $npds_forbidden_words = [];
            $npds_forbidden_words = $this->prepareForbiddenWords($npds_forbidden_words);
            $npds_forbidden_words = array_merge($npds_forbidden_words, $this->sanitize());

            $Xstring = preg_replace(
                array_keys($npds_forbidden_words), 
                array_values($npds_forbidden_words), 
                $Xstring
            );
        }
    ...
    /**
     * [removeMinisite description]
     *
     * @param   string  $Xstring  [$Xstring description]
     *
     * @return  string
     */
    public function removeMinisite(string $Xstring)
    {
        $npds_forbidden_words = [];
        $npds_forbidden_words = $this->prepareForbiddenWords($npds_forbidden_words);
        $npds_forbidden_words = array_merge($npds_forbidden_words, $this->sanitizeMinisite());
    ...
    private function sanitizeMinisite()
    {
        ...
        if ($tiny_mce) {
            ...
            if (!$blog_editor) {
                $tiny_mce_theme = 'full';
                $blog_editor = $this->editeur->aff_editeur('tiny_mce', 'begin') . $this->editeur->aff_editeur('story', 'false') . $this->editeur->aff_editeur('tiny_mce', 'end');
            }
        }
        ...
        return [
            ...
            "'!l_blog_ajouterOK!'i"                 => '<a class="list-group-item list-group-item-action" href="'. site_url('minisite.php?op=' . $op . '&amp;action=A') . '">
                                                            <i class="fas fa-pencil-alt fa-lg me-2"></i> 
                                                                ' . translate("Ajouter un article") . '
                                                        </a>',
            "'\<\?php'i"                            => "&lt;?php",
            "'\<\?'i"                               => "&lt;?",
            "'\?\>'i"                               => "?&gt;",
     ...
     * @return  array
     */
    private function codeHalfwidth()
    {
        // 2 dernières séquences = NCR (dec|hexa) correspondant aux caractères latin du bloc unicode Halfwidth and Fullwidth Forms.
